...
def recommendation_system_page():
    ...
    if st.button('Öner'):
        ...
        user_input = st.text_input("Puanınızı Girin", type="default")
        st.write("1-5 arası puanlayınız!")
    
        # Değerlendirme butonu
        evaluate_button_clicked = st.button("Değerlendir")
    
        # Kullanıcının değer girmesi ve değerlendirme butonuna tıklaması durumunda teşekkür mesajı gösterilecek
        if user_input and evaluate_button_clicked:
